...
upload_dataset_to_pennsieve() {
    ...
    for attempt in $(seq 1 $max_retries); do
        ...
        if pennsieve upload manifest "$manifest_id" </dev/null >>"$LOG_FILE" 2>&1; then
...
main() {
    ...
    # Find all PREVeNT Trial * folders
    local dataset_folders=()
    while IFS= read -r -d '' folder; do
        dataset_folders+=("$folder")
    done < <(find "$OUTPUT_DIR" -maxdepth 1 -type d -name "PREVeNT Trial *" -print0 | sort -z)
    ...
    # Generate channel count checker CSV
    generate_channel_count_csv
...
# --- Generate Channel Count Checker CSV ---
generate_channel_count_csv() {
    local csv_file="$LOG_DIR/channelCountChecker.csv"

    # Write CSV header
    echo "patient-id,session-dir,EEGChannelCount,ECGChannelCount,EMGChannelCount,EOGChannelCount,MiscChannelCount,TriggerChannelCount" > "$csv_file"

    # Find all eeg.json files and extract channel counts
    while IFS= read -r -d '' json_file; do
        # Extract patient ID (sub-XXXX) and session (ses-XXXX) from path
        local patient_id session_dir
        patient_id=$(echo "$json_file" | grep -oE 'sub-[^/]+' | head -1)
        session_dir=$(echo "$json_file" | grep -oE 'ses-[^/]+' | head -1)

        # Extract channel counts using python for reliable JSON parsing
        local counts
        counts=$(python -c "
import json
import sys
with open('$json_file', 'r') as f:
    data = json.load(f)
print(','.join([
    str(data.get('EEGChannelCount', '')),
    str(data.get('ECGChannelCount', '')),
    str(data.get('EMGChannelCount', '')),
    str(data.get('EOGChannelCount', '')),
    str(data.get('MiscChannelCount', '')),
    str(data.get('TriggerChannelCount', ''))
]))
" 2>/dev/null)

        if [[ -n "$counts" ]]; then
            echo "$patient_id,$session_dir,$counts" >> "$csv_file"
        fi
    done < <(find "$OUTPUT_DIR" -name "*_eeg.json" -print0 | sort -z)

    log_message "Channel count checker CSV generated: $csv_file"
}
